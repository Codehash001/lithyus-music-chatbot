...
function ChatMessageContent({
  ...
  const contents: ContentDisplayConfig[] = [
    ...
    {
      order: -3,
      component: eventData.length > 0 ? null : null,
    },
    {
      order: 2,
    ...
    {
      order: 3,
      component: sourceData[0] ? null : null,
    },
    {
      order: 4,
...
export default function ChatMessage({
  ...
  const messageClass = isAssistant ? 'bg-gray-300 ' : 'bg-black text-white';
  const containerClass = isAssistant ? 'justify-start' : 'justify-end';

  return (
    <div className={`flex items-start gap-2 p-2 ${containerClass}`}>
      {isAssistant && <ChatAvatar role={chatMessage.role} />}
      <div className={`flex flex-col ${messageClass} rounded-lg p-3 max-w-[85%] `}>
        <ChatMessageContent
          message={chatMessage}
          isLoading={isLoading}
          append={append}
